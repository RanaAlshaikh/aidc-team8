...
Racks: a DGX H100 node is 8 GPUs, 640 GB, 10.2 kW, four to a rack. A GB300 NVL72 rack is 72 GPUs, about 20 TB, about 120 kW.  
Serving: a model's weights at fp8, plus context for 32 conversations of 128K tokens.  
Training: 6 operations per parameter per token, 20 tokens per parameter, 40% of peak. An H100 peaks at 989 TFLOPS.  
Bill: the site draws 65% of its connection on average; USD 0.08 per kWh (30 halalas), or 0.048 at the industrial band.  
Cost: 125 output tokens per second per GPU; USD 450,000 per MW per month for everything but electricity.
...
remove 10% for switches : 80 x 0.10 = 8MW  
then 80 - 8 = 72 MW  

keep 20% headroom : 72 x 0.20 = 14.4MW  
...
45,176 × (989 TFLOPS × 40%) × 15.8M seconds ≈ 2.82 × 10^26 operations.  
...
In USD: 46,800,000 kWh x $0.08/kWh = $3,744,000  
...
GPUs (from Q1): 45,176  
45,176 x 125 tokens/s/GPU = 5,647,000 tokens/s  
5,647,000 tokens/s x 2,592,000 s/month (30 × 24 × 3600) = ~14.64 trillion tokens/month  
